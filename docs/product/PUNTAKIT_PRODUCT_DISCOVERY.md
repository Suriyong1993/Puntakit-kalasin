...
- ผมอ่านโค้ดเท่านั้น ผมไม่ได้รันแอป ไม่ได้รัน test และไม่ได้เชื่อมต่อฐานข้อมูล production
...
4. **หน้าเช็คชื่อแบบรายชื่อ (Live roster) และการเพิ่มสมาชิกเข้ากลุ่มมีข้อบกพร่องในโค้ด** client ขอ `limit` เกินค่าที่ server อนุญาต และอ่าน field ที่ server ไม่ส่งกลับ (E-30, E-31) ผลที่คาดคือรายชื่อว่าง (I-05) ยังไม่ได้ยืนยันด้วยการรันจริง
...
3. รายชื่อสมาชิกให้เลือกมาจาก `GET /api/members?limit=200` ซึ่ง server ปฏิเสธ (สูงสุด 100) และ client อ่าน `.items` ซึ่ง server ไม่ส่ง (E-31) ผลที่คาด: รายชื่อให้เลือกว่าง

### W4 — เช็คชื่อ

1. เจ้าหน้าที่เปิด `/attendance` เลือกประเภท, วันที่, กลุ่ม
2. แท็บ Live roster โหลด `GET /api/members?limit=300` และ `GET /api/attendance?limit=300` ทั้งสองค่าเกินขีดจำกัดของ server (100 และ 200) (E-30)
...
- **I-05** จาก E-30, E-31: หน้า Live roster และตัวเลือกสมาชิกในหน้ากลุ่มน่าจะแสดงรายชื่อว่าง ถ้าเป็นจริง ข้อมูล `group_members` และ `attendance_records` ใน production (ถ้ามี) น่าจะมาจากช่องทาง QR หรือการแก้ฐานข้อมูลโดยตรงเท่านั้น
...
- **U-11** ข้อบกพร่อง E-30/E-31 เกิดจริงในเบราว์เซอร์หรือไม่ (ยังไม่ได้รัน)
...
| P-03 | เจ้าหน้าที่ | J-04 | หน้าเช็คชื่อแบบรายชื่อน่าจะไม่โหลดรายชื่อ | E-30, E-31 | Evidence ในโค้ด; ยังไม่รันยืนยัน (U-11) |
...
3. **ยืนยันข้อบกพร่องที่มีหลักฐานในโค้ด** (E-30, E-31) ด้วยการรันแอปในเครื่อง ไม่แก้โค้ดในขั้นนี้
